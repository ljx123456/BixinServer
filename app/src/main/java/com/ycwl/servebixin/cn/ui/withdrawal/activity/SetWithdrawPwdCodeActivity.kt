...
class SetWithdrawPwdCodeActivity: BaseActivity(), ResetPwdCodeView {
    ...
    override fun initActivityData() {
        presenter.getSendCode(SendCodeBody(user.getUserPhone(),user.getUserToken(),8))
        codeTime.codeCountTimer(set_withdraw_password_sendAgainCode)
        tv_set_withdraw_password_code_phone.text="请输入"+user.getUserPhone()+"收到的验证码以设置提现密码"
    }
    ...
    override fun clickListener() {
        ...
        pc_set_withdraw_password_code.setOnInputListener(object : PhoneCode.OnInputListener{
            override fun onSucess(code: String?) {
                presenter.getValidationCode(ValidationCodeBody(user.getUserPhone(),pc_set_withdraw_password_code.phoneCode,8))
            }
    ...
    override fun getValidationCodeRequest() {
        intentUtils.intentSetWithdrawPwd()
    }
